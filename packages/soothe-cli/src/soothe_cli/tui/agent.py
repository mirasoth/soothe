...
from soothe_cli.tui.configurable_model import ConfigurableModelMiddleware
from soothe_cli.tui.integrations.sandbox_factory import get_default_working_dir
from soothe_cli.tui.local_context import (
    LocalContextMiddleware,
    _AsyncExecutableBackend,
    _ExecutableBackend,
)
from soothe_cli.tui.project_utils import ProjectContext, get_server_project_context
from soothe_cli.tui.subagents import list_subagents
...
class ShellAllowListMiddleware(AgentMiddleware):
    ...
    **before execution**. Rejected commands are returned as error `ToolMessage`
    ...
    def __init__(self, allow_list: list[str]) -> None:
        ...
        """
        from soothe_cli.tui.config import SHELL_ALLOW_ALL
    ...
    def _validate_tool_call(self, request: ToolCallRequest) -> ToolMessage | None:
        ...
        from soothe_cli.tui.config import SHELL_TOOL_NAMES, is_shell_command_allowed
...
def _format_execute_description(
    ...
    effective_cwd = (
        str(project_context.user_cwd) if project_context is not None else str(Path.cwd())
    )
...
def create_cli_agent(
    ...
    *,
    ...
    if restrictive_shell_allow_list is not None:
        ...
        if not any(
            subagent["name"] == GENERAL_PURPOSE_SUBAGENT["name"] for subagent in custom_subagents
        ):
    ...
    # read the channel.
    from soothe_cli.tui.token_state import TokenStateMiddleware

    agent_middleware.append(TokenStateMiddleware())

    # Add ask_user middleware (must be early so its tool is available)
    if enable_ask_user:
        from soothe_cli.tui.ask_user import AskUserMiddleware
