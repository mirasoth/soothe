...
from soothe_daemon.plan.rich_tree import render_plan_tree
    ...
    if normalized in {"all", "actions"}:
        ...
        if actions:
            ...
            for record in actions:
                ...
                source = (
                    ":".join(namespace) if isinstance(namespace, list) and namespace else "main"
                )
...
async def _handle_thread_command(
    console: Console, runner: SootheRunner, subcommand: str, parts: list[str]
) -> None:
    if not subcommand:
