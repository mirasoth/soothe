...
if TYPE_CHECKING:
    from soothe_daemon.protocols.vector_store import VectorStoreProtocol
...
def create_vector_store(
    ...
    if provider in ("in_memory", "none"):
        from soothe_daemon.backends.vector_store.in_memory import InMemoryVectorStore

        return InMemoryVectorStore(collection=collection)

    if provider == "pgvector":
        from soothe_daemon.backends.vector_store.pgvector import PGVectorStore

        return PGVectorStore(collection=collection, **config)

    if provider == "weaviate":
        from soothe_daemon.backends.vector_store.weaviate import WeaviateVectorStore

        return WeaviateVectorStore(collection=collection, **config)

    if provider == "sqlite_vec":
        from soothe_daemon.backends.vector_store.sqlite_vec import SQLiteVecStore
