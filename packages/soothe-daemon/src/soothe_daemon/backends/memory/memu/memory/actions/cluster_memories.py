"""Cluster Memories Action.

Clusters memories into different categories based on content analysis.
"""
...
class ClusterMemoriesAction(BaseAction):
    ...
    def execute(
        ...
        new_theory_of_mind_items: list[dict[str, str]] | None = None,
        session_date: str | None = None,
    ) -> dict[str, Any]:
        """Cluster memories into different categories."""
        if new_theory_of_mind_items is None:
    ...
    def _format_memory_item(self, memory_item: dict[str, str]) -> str:
        ...
        return (
            f"[{memory_item['memory_id']}][mentioned at {memory_item['mentioned_at']}] "
            f"{memory_item['content']} [{memory_item['memory_id']}]"
        )
...
You are given a list of existing clusters, a list of memory items, and the full conversation context
that generated these memories.
...
2. It is possible that a memory item is related to multiple clusters.
   Example: "We went to hiking in Blue Ridge Mountains this summer" is related to both "hiking"
   and "summer events" clusters, if both these two clusters are in the Existing Clusters.
3. If it possible that some memory items are not related to any of the existing clusters,
   you don't need to force them into any cluster.
4. DO NOT output memory items that are not related to any of the existing clusters.
5. Consider the conversation context when determining relationships - topics discussed together
   might belong to the same cluster.
...
"""
    ...
    def _detect_new_clusters(
        ...
        system_message = f"""You are an expert in discovering important or repeating events in one's memory records.

You are given a conversation context, a list of memory items extracted from this conversation,
...
4. The Event Name should be short and clear. A single word is the best (e.g., "marriage", "hiking").
   Never let the name be longer than 3 words.
5. The Event Name should contains only alphabets or space. DO NOT use any other characters
   including hyphen, underscore, etc.
6. An event can be considered repeating, periodical, or routine, if they are mentioned at least
   twice in the memory items OR if the conversation context suggests it's a recurring theme.
7. If an event is considered important enough (e.g., proposal), you should record it no matter
   how many times it is mentioned.
8. For event content that are close (e.g., hiking and backpacking), you can merge them into a
   single event, and accumulate the count.
...
"""
        ...
        for line in response.split("\n"):
            ...
            for memory_id_raw in memory_ids.split(","):
