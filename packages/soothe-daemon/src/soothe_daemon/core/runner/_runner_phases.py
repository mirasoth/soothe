...
from soothe_daemon.protocols.planner import PlanContext, StepResult
from soothe_daemon.protocols.policy import ActionRequest, PolicyContext
...
class PhasesMixin:
    ...
    async def _run_chitchat(
        ...
        from soothe_daemon.core.unified_classifier import _looks_chinese
    ...
    async def _run_direct_subagent(
        ...
        """
        from soothe_daemon.core.unified_classifier import RoutingResult, UnifiedClassification
    ...
    async def _pre_stream_independent(
        ...
        from soothe_daemon.protocols.durability import ThreadMetadata
        ...
        if self._policy:
            try:
                from soothe_daemon.protocols.policy import PermissionSet
    ...
    async def _collect_context_for_injection(self, state: Any) -> None:
        ...
        from soothe_daemon.core.workspace import get_git_status
...
async def generate_final_report_from_checkpoint(
    ...
    from soothe_daemon.cognition.agent_loop.result_cache import ToolResultCache
...
Format the report with clear sections and bullet points where appropriate."""
    ...
    try:
        ...
        logger.info(
            "Generated final report from checkpoint (goal: %s, report length: %d chars)",
            goal[:50],
            len(final_report),
        )
