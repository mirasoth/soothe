...
from soothe_daemon.cognition.agent_loop import AgentLoop
from soothe_daemon.cognition.agent_loop.schemas import PlanResult
...
from soothe_daemon.protocols.planner import StepResult
...
class AutonomousMixin(GoalDirectivesMixin):
    ...
    async def initialize_autopilot(self, soothe_home: Path) -> None:
        ...
        """
        from soothe_daemon.cognition.goal_engine.discovery import discover_goals
    ...
    async def _run_autonomous(
        ...
        *,
        ...
        if self._unified_classifier:
            from soothe_daemon.core.unified_classifier import UnifiedClassification
        ...
        from soothe_daemon.cognition.goal_engine.proposal_queue import ProposalQueue
        ...
        *,
        ...
        if self._planner and hasattr(self._planner, "plan"):
            ...
            if goal_result:
                ...
                # Update goal report
                from soothe_daemon.protocols.planner import GoalReport
                ...
                if (
                    ...
                    try:
                        from soothe_daemon.protocols.memory import MemoryItem
                ...
                if self._planner and self._goal_engine:
                    try:
                        from soothe_daemon.protocols.planner import GoalContext, GoalSnapshot
        ...
        try:
                ...
                try:
                    ...
                    if step_results:
                        ...
                        if self._goal_engine:
                            from soothe_daemon.protocols.planner import GoalContext
            ...
            if self._goal_engine and self._model and not should_continue:
                from soothe_daemon.cognition.goal_engine.consensus import evaluate_goal_completion
            ...
            from soothe_daemon.utils.error_format import emit_error_event
    ...
    async def _handle_suggested_goal(self, proposal: Any) -> None:
        ...
        """
        from soothe_daemon.cognition.goal_engine.criticality import evaluate_criticality_async
    ...
    async def _queue_must_confirmation(
        ...
        try:
            from soothe_daemon.cognition.channel.models import ChannelMessage
            from soothe_daemon.cognition.channel.outbox import ChannelOutbox
    ...
    async def _send_autopilot_webhook(self, event_type: str, payload: dict) -> None:
        ...
        """
        try:
            from soothe_daemon.cognition.goal_engine.webhooks import WebhookConfig, WebhookService
    ...
    async def _detect_relationships_for_goal(self, completed_goal: Any) -> list[dict]:
        ...
        try:
            from soothe_daemon.cognition.goal_engine.relationship_detector import (
    ...
    async def _check_scheduled_and_dream(
        ...
        try:
            from soothe_daemon.cognition.scheduler import SchedulerService
        ...
        try:
            from soothe_daemon.cognition.goal_engine.dreaming import DreamingMode
