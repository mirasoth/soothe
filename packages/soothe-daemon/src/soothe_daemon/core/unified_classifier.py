...
class RoutingResult(BaseModel):
    ...
    task_complexity: Literal["chitchat", "medium", "complex"] = Field(
        description="chitchat | medium | complex"
    )
...
class UnifiedClassifier:
    ...
    def __init__(
        ...
        if fast_model:
            ...
            if config and hasattr(config, "llm_tracing") and config.llm_tracing.enabled:
                from soothe_daemon.core.middleware._wrapper import LLMTracingWrapper
    ...
    async def classify_routing(
        ...
        *,
        ...
        if result.task_complexity == "chitchat" and not result.chitchat_response:
            ...
            logger.debug(
                "Patched missing chitchat_response for query: %s", preview_first(query, 50)
            )
    ...
    async def _llm_routing(
        ...
        *,
        ...
        if not result.task_complexity or result.task_complexity not in (
            "chitchat",
            "medium",
            "complex",
        ):
    ...
    def _create_routing_model(self, fast_model: BaseChatModel) -> Any:
        ...
        for method in methods:
            try:
                ...
                logger.debug(
                    "with_structured_output init failed for method=%s", method, exc_info=True
                )
