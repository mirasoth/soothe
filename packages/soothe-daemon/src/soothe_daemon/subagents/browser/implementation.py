...
from soothe_daemon.subagents.browser.events import (
    BrowserCdpEvent,
...
def _build_browser_graph(
    *,
    ...
    async def _run_browser_async(state: dict[str, Any]) -> dict[str, Any]:
        ...
        from soothe_daemon.utils.runtime import (
        ...
        try:
            with capture_subagent_output("browser", suppress=True):
                ...
                if browser_config.enable_existing_browser:
                    ...
                    if use_existing:
                        from soothe_daemon.utils.browser_cdp import find_available_cdp
                ...
                if not cdp_url:
                    from soothe_daemon.utils.browser_cdp import cleanup_stale_chrome
                ...
                if browser_config.cleanup_on_exit:
                    from soothe_daemon.utils.runtime import cleanup_browser_temp_files

                    cleanup_browser_temp_files()
        except Exception as e:
            logger.exception("Browser agent failed")
            from soothe_daemon.utils.error_format import format_cli_error
