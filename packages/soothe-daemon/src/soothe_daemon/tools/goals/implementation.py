...
class CreateGoalTool(BaseTool):
    ...
    description: str = "Create a new goal. Parameters: description (required), parent_id (optional). Returns: goal ID and details."
    goal_engine: GoalEngine = Field(exclude=True)

    def _run(
        self, description: str = "", priority: int = 50, parent_id: str = ""
    ) -> dict[str, Any]:
        """Create a new goal.
        ...
        async def _create() -> dict[str, Any]:
            goal = await self.goal_engine.create_goal(
                description, priority=priority, parent_id=parent_id or None
            )
...
class ListGoalsTool(BaseTool):
    ...
    def _run(self, status: str = "") -> dict[str, Any]:
        ...
        async def _list() -> dict[str, Any]:
            filter_status = (
                status if status in ("pending", "active", "completed", "failed") else None
            )
...
class ReportProgressTool(BaseTool):
    ...
    description: str = "Report progress on the current goal. Parameters: goal_id (required), status, findings. Returns: confirmation."
    goal_engine: GoalEngine = Field(exclude=True)
    ...
    async def _arun(
        ...
        if self.proposal_queue:
            from soothe_daemon.cognition.goal_engine.proposal_queue import Proposal
