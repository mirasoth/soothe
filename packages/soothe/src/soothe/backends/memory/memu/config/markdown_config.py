...
class MarkdownConfigManager:
    ...
    def _load_all_configs(self) -> None:
        ...
        if not self.yaml_path.exists():
            print(f"Warning: YAML configuration file not found: {self.yaml_path}")  # noqa: T201
        ...
        try:
            ...
            print(f"Warning: Failed to parse YAML configuration {self.yaml_path}: {e}")  # noqa: T201
            self._processing_order = []
            return
    ...
    def get_file_config(self, file_type: str) -> MarkdownFileConfig | None:
...
_config_manager: MarkdownConfigManager | None = None
...
def detect_file_type(filename: str, content: str = "") -> str:  # noqa: ARG001
    ...
    # Detect profile type
    profile_keywords = ["profile", "personal_info", "bio", "resume"]
    if any(keyword in filename_lower for keyword in profile_keywords) and "profile" in file_types:
        return "profile"

    # Detect event type
    event_keywords = ["event", "events", "activity", "milestone"]
    if any(keyword in filename_lower for keyword in event_keywords) and "event" in file_types:
        return "event"

    # Detect activity type
    activity_keywords = ["activity", "activities", "daily", "diary", "log"]
    if any(keyword in filename_lower for keyword in activity_keywords) and "activity" in file_types:
        return "activity"
...
def get_all_file_configs() -> dict[str, MarkdownFileConfig]:
    ...
    manager = get_config_manager()
    return {
        file_type: manager.get_file_config(file_type) for file_type in manager.get_all_file_types()
    }
