...
| `soothe autopilot "task"` | Autonomous execution mode. |
...
| `soothe config` | Show current configuration (default action). |
| `soothe config -i` | Initialize `$SOOTHE_HOME` (config, threads, logs). |
| `soothe config --validate` | Validate configuration file. |
| `soothe thread` | List all threads (default action). |
| `soothe thread -l` | List all threads (explicit). |
| `soothe thread <id>` | Show thread details. |
| `soothe thread -c <id>` | Continue a thread (launches TUI). |
| `soothe thread -a <id>` | Archive a thread. |
| `soothe thread -d <id>` | Permanently delete a thread. |
| `soothe thread -e <id>` | Export thread to jsonl or md. |
| `soothe thread --stats <id>` | Show thread statistics. |
| `soothe agent` | List available agents (default action). |
