...
1. Daemon ignores SIGTERM so `soothe server stop` fails.
...
- `soothe server stop` terminates daemon within 10s.
- `soothe server start` twice prints "already running" on second attempt.
