...
def build_soothe_workspace_section(
    ...
    *,
    ...
    if git_status:
        ...
        lines.append(f"  <main_branch>{_xml_text(git_status.get('main_branch', 'main'))}</main_branch>")
    ...
    if include_layout_preview and root.is_dir():
        ...
        if preview:
            lines.append(f'<layout_preview max_entries="{max_layout_entries}">{_xml_text(preview)}</layout_preview>')
...
def build_soothe_protocols_section(protocol_summary: dict[str, Any]) -> str:
    ...
    for proto_name in proto_names:
        ...
        entries.append(f'<protocol id="{_xml_attr(proto_name)}" type="{_xml_attr(proto_type)}"{stat_attr}/>')
    if not entries:
        return ""
    inner = "\n".join(entries)
    ver = RFC104_CONTEXT_XML_VERSION
