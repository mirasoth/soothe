        ...
        async with self._read_lock:
            try:
                while True:
                    ...
                    data = event.get("data")
                    if isinstance(data, dict) and data.get("type") == "soothe.lifecycle.daemon.heartbeat":
                        continue
                    ...
                    if mode == "updates" and isinstance(normalized, dict) and "__interrupt__" in normalized:
                        break
            finally:
                self._streaming = False
