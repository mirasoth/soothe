...
if TYPE_CHECKING:
    ...
    from soothe.ux.tui.output import OutputFormat
...
from soothe.ux.tui.configurable_model import ConfigurableModelMiddleware
from soothe.ux.tui.integrations.sandbox_factory import get_default_working_dir
from soothe.ux.tui.local_context import (
    LocalContextMiddleware,
    _AsyncExecutableBackend,
    _ExecutableBackend,
)
from soothe.ux.tui.project_utils import ProjectContext, get_server_project_context
from soothe.ux.tui.subagents import list_subagents
from soothe.ux.tui.unicode_security import (
    check_url_safety,
...
class ShellAllowListMiddleware(AgentMiddleware):
    ...
    **before execution**. Rejected commands are returned as error `ToolMessage`
    ...
    def __init__(self, allow_list: list[str]) -> None:
        ...
        """
        from soothe.ux.tui.config import SHELL_ALLOW_ALL
    ...
    def _validate_tool_call(self, request: ToolCallRequest) -> ToolMessage | None:
        ...
        from soothe.ux.tui.config import SHELL_TOOL_NAMES, is_shell_command_allowed
...
def list_agents(*, output_format: OutputFormat = "text") -> None:
    ...
    if not agents_dir.exists() or not any(agents_dir.iterdir()):
        if output_format == "json":
            from soothe.ux.tui.output import write_json
...
def _format_web_search_description(tool_call: ToolCall, _state: AgentState[Any], _runtime: Runtime[Any]) -> str:
    ...
    return f"Query: {query}\nMax results: {max_results}\n\n{get_glyphs().warning}  This will use Tavily API credits"
...
def create_cli_agent(
    ...
    *,
    ...
    tools = tools or []
    effective_cwd = (
        Path(cwd) if cwd is not None else (project_context.user_cwd if project_context is not None else None)
    )
    ...
    if enable_skills:
        ...
        project_skills_dir = (
            project_context.project_skills_dir() if project_context is not None else settings.get_project_skills_dir()
        )
    ...
    project_agents_dir = (
        project_context.project_agents_dir() if project_context is not None else settings.get_project_agents_dir()
    )
    ...
    # read the channel.
    from soothe.ux.tui.token_state import TokenStateMiddleware

    agent_middleware.append(TokenStateMiddleware())

    # Add ask_user middleware (must be early so its tool is available)
    if enable_ask_user:
        from soothe.ux.tui.ask_user import AskUserMiddleware
    ...
    if auto_approve or shell_middleware_added:  # noqa: SIM108  # if-else clearer than ternary for dual-path config
        ...
        _add_interrupt_on()  # type: ignore[assignment]  # InterruptOnConfig is compatible at runtime
