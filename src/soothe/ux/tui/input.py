...
from soothe.ux.tui.config import console
from soothe.ux.tui.media_utils import ImageData, VideoData
    ...
    for match in matches:
        ...
        try:
            ...
            if resolved.exists() and resolved.is_file():
                ...
                console.print(f"[yellow]Warning: File not found: {escape_markup(raw_path)}[/yellow]")
        except (OSError, RuntimeError) as e:
            console.print(f"[yellow]Warning: Invalid path {escape_markup(raw_path)}: {escape_markup(str(e))}[/yellow]")
...
def normalize_pasted_path(text: str) -> Path | None:
    ...
    unquoted = (
        payload.removeprefix('"').removesuffix('"') if payload.startswith('"') and payload.endswith('"') else payload
    )
    unquoted = (
